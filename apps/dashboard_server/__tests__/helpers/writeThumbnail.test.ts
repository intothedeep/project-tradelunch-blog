...
describe('parseThumbnailUrl', () => {
    ...
    it('maps jpg/webp/gif extensions to their mime types', () => {
        expect(
            parseThumbnailUrl('https://blog-assets.prettylog.com/7/a.jpg', cfg)
                ?.contentType
        ).toBe('image/jpeg');
        expect(
            parseThumbnailUrl('https://blog-assets.prettylog.com/7/a.webp', cfg)
                ?.contentType
        ).toBe('image/webp');
        expect(
            parseThumbnailUrl('https://blog-assets.prettylog.com/7/a.gif', cfg)
                ?.contentType
        ).toBe('image/gif');
    });
    ...
    it('returns null when the prefix is present but no key follows', () => {
        expect(
            parseThumbnailUrl('https://blog-assets.prettylog.com/', cfg)
        ).toBeNull();
    });

    it('falls back to octet-stream for an unknown extension', () => {
        expect(
            parseThumbnailUrl('https://blog-assets.prettylog.com/1/x.bmp', cfg)
                ?.contentType
