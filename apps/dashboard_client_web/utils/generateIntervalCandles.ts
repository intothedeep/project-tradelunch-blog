...
const INTRADAY_SECONDS: Record<Exclude<ChartInterval, 'D' | 'W' | 'M'>, number> = {
  '1m': 60,
  '5m': 300,
  '15m': 900,
  '30m': 1800,
...
function aggregate(candles: IOHLCPoint[], bucketKey: (time: string) => string): IOHLCPoint[] {
...
function buildIntraday(label: string, intervalSec: number, endValue: number): IOHLCPoint[] {
  ...
  for (let i = 0; i < INTRADAY_COUNT; i++) {
    ...
    const close = i === INTRADAY_COUNT - 1
      ? endValue
