...
export function useChartDrawings({
  chartRef, candleSeriesRef, chartReady, candles, label, interval, enabled, haloColor,
}: Params): void {
  ...
  const resolvePoint = useCallback(
    (param: MouseEventParams<Time>): DrawingPoint | null => {
      ...
      const candidates = magnet === 'strong'
  ...
  useEffect(() => {
    ...
    drawings, key, resolvePoint, setCursor, setSelectedId, setInProgress,
    setDrawingsByKey,
  ])
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        setActiveTool(null)
        setSelectedId(null)
      } else if ((e.key === 'Delete' || e.key === 'Backspace') && selectedId !== null && key !== null) {
        setDrawingsByKey((prev) => ({
          ...prev,
  ...
  }, [enabled, selectedId, key, setInProgress, setActiveTool, setSelectedId, setDrawingsByKey])
  ...
  useEffect(() => {
    ...
    return () => {
      try { series.detachPrimitive(primitive) } catch { /* chart already removed */ }
...
function buildDrawing(
  ...
  switch (kind) {
    ...
    case 'parallel_channel':
      return p1 && p2 && p3
        ? { id, kind, color, lineWidth, p1, p2, p3, fillColor: 'rgba(41,98,255,0.12)' }
        : null
    case 'fib_retracement':
      return p1 && p2 ? { id, kind, color, lineWidth, p1, p2 } : null
    case 'fib_extension':
      return p1 && p2 && p3 ? { id, kind, color, lineWidth, p1, p2, p3 } : null
  }
}
