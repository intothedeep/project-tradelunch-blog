...
export default function ChartLegend({
    ...
    const { maArrays, rsiArr, macdResult, maVisible, rsiVisible, macdVisible } =
        indicators;
                ...
                {MA_PERIODS.map((p) => {
                    ...
                    return (
                        <span
                            ...
                            className="tabular-nums"
                            style={{
                                color: isOn ? maColors[p] : labelColor,
                                opacity: isOn ? 1 : 0.5,
                            }}
                        >
                            MA{p} {text}
            ...
            {(rsiVisible || macdVisible) && (
                <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5">
                    {rsiVisible && (
                        <span
                            className="tabular-nums"
                            style={{ color: palette.rsi }}
                        >
                            RSI(14){' '}
                            {rsiAt !== null ? formatOscillator(rsiAt) : '—'}
                        </span>
                    )}
                    {macdVisible && (
                        <>
                            <span
                                className="tabular-nums"
                                style={{ color: palette.macd }}
                            >
                                MACD{' '}
                                {macdAt !== null
                                    ? formatOscillator(macdAt)
                                    : '—'}
                            </span>
                            <span
                                className="tabular-nums"
                                style={{ color: palette.macdSignal }}
                            >
                                Signal{' '}
                                {signalAt !== null
                                    ? formatOscillator(signalAt)
                                    : '—'}
                            </span>
                            <span
                                className="tabular-nums"
