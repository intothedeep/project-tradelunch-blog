...
export default function ChartConfigMenu({
  containerRef, outerRef, palette,
  maVisible, rsiVisible, macdVisible, ichimokuVisible,
  onToggleMA, onToggleRSI, onToggleMACD, onToggleIchimoku,
  onClose, initialPos,
}: Props) {
  ...
  return (
    ...
    >
      <div
        ...
        style={{ borderColor: palette.gridLine, color: palette.textSecondary }}
      >
      ...
      <div className="px-2 py-1 border-b" style={{ borderColor: palette.gridLine, color: palette.textSecondary }}>
        MOVING AVERAGES
      </div>
      <div className="flex flex-col">
        {MA_PERIODS.map((p) => (
          ...
          >
            <input type="checkbox" checked={maVisible[p]} onChange={() => onToggleMA(p)} className="cursor-pointer" />
            <span className="inline-block w-3 h-0.5" style={{ backgroundColor: maColors[p] }} />
            <span style={{ color: maColors[p] }}>MA{p}</span>
          </label>
        ))}
      </div>
      <div className="px-2 py-1 border-y" style={{ borderColor: palette.gridLine, color: palette.textSecondary }}>
        OSCILLATORS
      </div>
      <div className="flex flex-col">
        <label className="flex items-center gap-2 px-2 py-1 cursor-pointer hover:bg-black/[0.05] dark:hover:bg-white/[0.05]">
          <input type="checkbox" checked={rsiVisible} onChange={onToggleRSI} className="cursor-pointer" />
          <span className="inline-block w-3 h-0.5" style={{ backgroundColor: palette.rsi }} />
          <span style={{ color: palette.rsi }}>RSI(14)</span>
        </label>
        <label className="flex items-center gap-2 px-2 py-1 cursor-pointer hover:bg-black/[0.05] dark:hover:bg-white/[0.05]">
          <input type="checkbox" checked={macdVisible} onChange={onToggleMACD} className="cursor-pointer" />
          <span className="inline-block w-3 h-0.5" style={{ backgroundColor: palette.macd }} />
      ...
      <div className="flex flex-col">
        <label className="flex items-center gap-2 px-2 py-1 cursor-pointer hover:bg-black/[0.05] dark:hover:bg-white/[0.05]">
          <input type="checkbox" checked={ichimokuVisible} onChange={onToggleIchimoku} className="cursor-pointer" />
          <span className="inline-flex items-center gap-px">
            <span className="inline-block w-1.5 h-0.5" style={{ backgroundColor: palette.ichimokuTenkan }} />
            <span className="inline-block w-1.5 h-0.5" style={{ backgroundColor: palette.ichimokuKijun }} />
            <span className="inline-block w-1.5 h-0.5" style={{ backgroundColor: palette.ichimokuSpanA }} />
            <span className="inline-block w-1.5 h-0.5" style={{ backgroundColor: palette.ichimokuSpanB }} />
          </span>
          <span style={{ color: palette.textPrimary }}>Ichimoku(9,26,52)</span>
        </label>
      </div>
    </div>
  )
