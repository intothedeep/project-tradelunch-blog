...
import { selectedIntervalAtom, selectedLabelAtom, selectedRangeAtom } from '@/store/dashboard.atom'
...
export default function ChartPanel({ history, className }: Props) {
  ...
  const [maVisible, setMaVisible] = useState<MAVisibility>({
    5: false, 20: false, 50: false, 100: false, 200: false,
  })
  ...
  useEffect(() => {
    const onError = (event: ErrorEvent) => {
      ...
      if (msg.includes('Object is disposed') && (src.includes('fancy-canvas') || src.includes('lightweight-charts'))) {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (gearRef.current?.contains(target) || menuRef.current?.contains(target)) return
      setMenuOpen(false)
    }
  ...
  const palette: ChartPalette = resolvedTheme === 'light' ? TV_LIGHT : TV_DARK

  const candles = useMemo<IOHLCPoint[]>(() => {
    if (selectedLabel === null) return []
    const baseDaily = history[selectedLabel] ?? []
    return generateIntervalCandles(selectedLabel, selectedInterval, baseDaily)
  }, [selectedLabel, selectedInterval, history])

  const maArrays = useMemo(
    () => ({
      5:   computeMA(candles.map((c) => c.close), 5),
      20:  computeMA(candles.map((c) => c.close), 20),
      50:  computeMA(candles.map((c) => c.close), 50),
      100: computeMA(candles.map((c) => c.close), 100),
      200: computeMA(candles.map((c) => c.close), 200),
    }),
    [candles],
  )

  const rsiArr = useMemo(() => computeRSI(candles.map((c) => c.close), 14), [candles])
  const macdResult = useMemo(() => computeMACD(candles.map((c) => c.close), 12, 26, 9), [candles])
  const ichimoku = useMemo(() => computeIchimoku(candles), [candles])

  const toggleMA = (p: MAPeriod) => setMaVisible((prev) => ({ ...prev, [p]: !prev[p] }))
  ...
  const indicators = useMemo<IndicatorState>(
    () => ({ maArrays, rsiArr, macdResult, ichimoku, maVisible, rsiVisible, macdVisible, ichimokuVisible }),
    [maArrays, rsiArr, macdResult, ichimoku, maVisible, rsiVisible, macdVisible, ichimokuVisible],
  )

  const { hoverIndex, paneRects, chartRef, candleSeriesRef, chartReady } = useTradingViewChart({
  ...
  return (
    <div className={`flex flex-col h-full bg-white dark:bg-[#131722] ${className ?? ''}`}>
      <ChartHeader
        label={selectedLabel}
        lastClose={lastClose}
        change={change}
      ...
      <div className="flex flex-1 min-h-0">
        <ChartDrawToolbar label={selectedLabel} interval={selectedInterval} />
