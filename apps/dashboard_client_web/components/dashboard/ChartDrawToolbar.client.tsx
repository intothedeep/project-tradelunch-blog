...
export default function ChartDrawToolbar({ label, interval }: Props) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 w-10 py-2 bg-white dark:bg-[#1e222d] border-r border-[#e0e3eb] dark:border-[#2a2e39]">
      <button
        type="button"
        onClick={() => { setActiveTool(null); setInProgress(null) }}
        title="Pointer"
        aria-label="Pointer"
      ...
      <button
        ...
        style={magnet === 'strong' ? { background: '#ef5350', color: 'white' } : undefined}
      >
        <Magnet size={14} />
      </button>
      <span className="block w-6 h-px bg-[#e0e3eb] dark:bg-[#2a2e39]" />
      <button
        ...
        className={cn(btn(false), drawings.length === 0 && 'opacity-30 cursor-not-allowed')}
      >
        <Undo2 size={14} />
      </button>
      <button
