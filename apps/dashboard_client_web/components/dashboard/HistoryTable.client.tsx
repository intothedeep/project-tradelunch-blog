...
import {
  Banknote, Bitcoin, BarChart3, Percent, Briefcase,
  type LucideIcon,
} from 'lucide-react'
...
function Section({ heading, items, selected, onSelect, isOpen, onToggle }: SectionProps) {
  return (
    <>
      <button
        type="button"
      ...
      >
        <span className="inline-block w-3 text-center">{isOpen ? '▼' : '▶'}</span>
        <span>{heading}</span>
      </button>
      {isOpen && items.map((item) => {
        const isPos = item.change.percent >= 0
        const numColor = isPos ? 'text-[#26a69a]' : 'text-[#ef5350]'
        ...
        return (
          ...
          >
            <div className="flex items-center gap-1.5 min-w-0">
              ...
              <span className="text-[#131722] dark:text-[#d1d4dc] text-xs font-semibold truncate">{item.label}</span>
            </div>
            <span className="text-[#131722] dark:text-[#d1d4dc] text-xs tabular-nums font-mono text-right">
              {formatLast(item.value)}
            </span>
            <span className={cn('text-xs tabular-nums font-mono text-right', numColor)}>
              {formatChg(item.change.absolute)}
            </span>
            <span className={cn('text-xs tabular-nums font-mono text-right w-14', numColor)}>
              {item.change.percent >= 0 ? '+' : ''}{item.change.percent.toFixed(2)}%
            </span>
          </div>
        )
      })}
...
const SECTIONS: readonly WatchlistSection[] = ['FX', 'Crypto', 'Indices', 'Rates', 'Stocks']
...
export default function HistoryTable({ snapshot, className }: Props) {
  ...
  const toggle = (key: WatchlistSection) => setOpen((prev) => ({ ...prev, [key]: !prev[key] }))
  ...
  return (
    <div className={cn('flex flex-col bg-white dark:bg-[#1e222d] text-[#131722] dark:text-[#d1d4dc] text-sm', className)}>
      <div className="sticky top-0 z-10 bg-white dark:bg-[#1e222d] border-b border-[#e0e3eb] dark:border-[#2a2e39]">
        <div className="grid grid-cols-[1fr_auto_auto_auto] gap-x-2 px-2 py-1.5">
          <span className="text-[10px] font-semibold tracking-widest text-[#787b86] uppercase">Watching</span>
          <span className="text-[10px] text-[#787b86] uppercase text-right">Last</span>
          <span className="text-[10px] text-[#787b86] uppercase text-right">Chg</span>
          <span className="text-[10px] text-[#787b86] uppercase text-right w-14">Chg%</span>
        </div>
      </div>
