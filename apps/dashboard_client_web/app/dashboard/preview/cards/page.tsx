...
function ItemCard({ item }: { item: IDashboardItem | IStockItem }) {
  ...
  return (
    <div className="rounded-lg border bg-card p-3 flex flex-col gap-1 min-w-0">
      ...
      <span className={cn('flex items-center gap-1 text-xs font-medium', changeColor)}>
...
export default function CardsPreviewPage() {
  ...
  return (
    <main className="p-4 md:p-8 max-w-screen-xl mx-auto">
      ...
      <CategorySection heading="FX" items={snap.fx.items} />
      <CategorySection heading="Crypto" items={snap.crypto.items} />
      <CategorySection heading="Indices" items={snap.indices.items} />
      <CategorySection heading="Interest Rates" items={snap.rates.items} />
      <CategorySection heading="Stocks" items={snap.stocks.items} />
    </main>
  )
}
