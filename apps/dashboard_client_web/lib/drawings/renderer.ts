...
export function renderDrawings(
  ...
  for (const d of input.drawings) {
    ...
    if (d.id === input.selectedId) drawSelectionHalo(ctx, d, chart, series, input.selectionHaloColor)
  }
...
function drawOne(
  ...
  switch (d.kind) {
    case 'horizontal_line':  return drawHorizontalLine(ctx, d, series, bounds)
    case 'vertical_line':    return drawVerticalLine(ctx, d, chart, bounds)
    case 'trend_line':       return drawTrendLine(ctx, d, chart, series)
    case 'ray':              return drawRay(ctx, d, chart, series, bounds)
    case 'parallel_channel': return drawParallelChannel(ctx, d, chart, series)
    case 'fib_retracement':  return drawFibRetracement(ctx, d, chart, series, bounds)
    case 'fib_extension':    return drawFibExtension(ctx, d, chart, series, bounds)
  }
}
...
function anchorPoints(d: Drawing): DrawingPoint[] {
  switch (d.kind) {
    ...
    case 'trend_line':      return [d.p1, d.p2]
    case 'ray':             return [d.p1, d.p2]
    case 'parallel_channel':return [d.p1, d.p2, d.p3]
    case 'fib_retracement': return [d.p1, d.p2]
    case 'fib_extension':   return [d.p1, d.p2, d.p3]
  }
}
...
function buildGhost(
  ...
  switch (ip.kind) {
    case 'horizontal_line':
      return { id, kind: 'horizontal_line', color, lineWidth, price: cursor.price }
    case 'vertical_line':
      return { id, kind: 'vertical_line', color, lineWidth, time: cursor.time }
    case 'trend_line': {
    ...
    case 'parallel_channel': {
      ...
      return {
        id, kind: 'parallel_channel', color, lineWidth,
    ...
    case 'fib_retracement': {
      const p1 = ip.points[0]
      if (!p1) return null
      return { id, kind: 'fib_retracement', color, lineWidth, p1, p2: cursor }
    }
    case 'fib_extension': {
      const p1 = ip.points[0]
      const p2 = ip.points[1]
      ...
      return { id, kind: 'fib_extension', color, lineWidth, p1, p2, p3: cursor }
    }
  }
}
